...
void readFile(map<double, vector<string>>& transactionAmount, map<string, vector<vector<string>>>& transactionInfo) {
    ...
    ifstream fin("resources/FraudulentTransaction.csv");
    if (!fin.is_open()) {
        ...
        if (command == 0) {
            ...
            if (dataStructureChoice == 1) {
                ...
                if (command == 1) {
                    ...
                    cout<<endl;
                    auto start = high_resolution_clock::now();
                    if (category == 1) {CategoryMax.searchCategory("clothing");}
                    else if (category == 2) {CategoryMax.searchCategory("electronics");}
                    else if (category == 3) {CategoryMax.searchCategory("health & beauty");}
                    else if (category == 4) {CategoryMax.searchCategory("home & garden");}
                    else if (category == 5) {CategoryMax.searchCategory("toys & games");}
                    auto stop = high_resolution_clock::now();
                    auto duration = duration_cast<microseconds>(stop - start);
                    cout << "This function execution took: " << duration.count() << " microsecond(s)" << endl;
                    cout<<endl;
                    ...
                    cout<<endl;
                    auto start = high_resolution_clock::now();
                    if (payment == 1) {MethodMax.searchPaymentMethod("bank transfer");}
                    else if (payment == 2) {MethodMax.searchPaymentMethod("credit card");}
                    else if (payment == 3) {MethodMax.searchPaymentMethod("debit card");}
                    else if (payment == 4) {MethodMax.searchPaymentMethod("PayPal");}
                    auto stop = high_resolution_clock::now();
                    auto duration = duration_cast<microseconds>(stop - start);
                    cout << "This function execution took: " << duration.count() << " microsecond(s)" << endl;
                    cout<<endl;
